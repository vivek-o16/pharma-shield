...
(() => {
  ...
  function populateNsqMonthOptions() {
    const select = el("#nsq-month-select");
    if (!select) return;

    const nsqRecords = (DB.records || []).filter(
      (r) => (r.category || "").toUpperCase() === "NSQ" && r.alertMonth && r.alertYear
    );

    const map = new Map();
    nsqRecords.forEach((r) => {
      const key = r.alertMonth + "|" + r.alertYear;
      if (!map.has(key)) {
        map.set(key, { month: r.alertMonth, year: r.alertYear, sortKey: alertSortKey(r), count: 0 });
      }
      map.get(key).count += 1;
    });

    const options = [...map.values()].sort((a, b) => b.sortKey - a.sortKey);

    select.innerHTML =
      '<option value="">All records (latest)</option>' +
      options
        .map(
          (o) =>
            `<option value="${escapeHTML(o.month)}|${o.year}">${escapeHTML(o.month)} ${o.year} (${o.count} NSQ)</option>`
        )
        .join("");
  }

  function initNsqMonthFilter() {
    const select = el("#nsq-month-select");
    if (!select) return;
    ...
    select.addEventListener("change", () => {
      ...
      renderRecentAlerts({ month, year: Number(yearStr) });
    });
  }
  ...
  function renderRecentAlerts(monthFilter = null) {
    const grid = el("#recent-grid");
    if (!grid) return;

    const countLabel = el("#nsq-filter-count");
    ...
    if (monthFilter) {
      records = records.filter(
        (r) =>
          (r.category || "").toUpperCase() === "NSQ" &&
          r.alertMonth === monthFilter.month &&
          Number(r.alertYear) === Number(monthFilter.year)
      );
    }

    const sorted = [...records].sort((a, b) => alertSortKey(b) - alertSortKey(a));
    const display = monthFilter ? sorted : sorted.slice(0, 6);

    if (countLabel) {
      countLabel.textContent = monthFilter
        ? `${sorted.length} NSQ alert${sorted.length === 1 ? "" : "s"} found for ${escapeHTML(monthFilter.month)} ${monthFilter.year}`
        : "";
    }

    if (display.length === 0) {
      grid.innerHTML = monthFilter
        ? `<p>No NSQ alerts found for ${escapeHTML(monthFilter.month)} ${monthFilter.year}.</p>`
