...
(() => {
  ...
  async function loadDrugData() {
    try {
      ...
      console.warn("Pharma Shield: could not load data/drugAlerts.json, using embedded fallback.", err);
      DB = FALLBACK_DATA;
    }
    updateStatistics();
    renderRecentAlerts();
  ...
  function findExactMatch(nameCompact, batchCompact) {
    return DB.records.find((r) => {
      ...
      const nameHit = nameCompact ? (rName.includes(nameCompact) || nameCompact.includes(rName)) : true;
  ...
  function searchDrug(nameRaw, batchRaw) {
    ...
    // Priority 1: exact medicine + batch match
    if (batchCompact) {
      const exact = findExactMatch(nameCompact, batchCompact);
      if (exact) return { type: "alert", record: exact, nameDisplay, batchDisplay };
    }

    // Priority 2: medicine has alert history, but batch didn't match (or wasn't given)
    ...
    // If only a batch number was entered (no name), check batch across all records
    if (!nameCompact && batchCompact) {
      const byBatchOnly = DB.records.find((r) => normalizeCompact(r.batchNumber) === batchCompact);
      if (byBatchOnly) return { type: "alert", record: byBatchOnly, nameDisplay, batchDisplay };
    }

    return { type: "clear", nameDisplay, batchDisplay };
  }
  ...
  function renderNoAlertResult(nameDisplay) {
    return `
      <article class="result-card status-clear">
        <span class="result-badge">✓ No alert found</span>
        <p class="result-message">No matching CDSCO quality alert was found in the available dataset${nameDisplay ? ` for “${escapeHTML(nameDisplay)}”` : ""}.</p>
        <p class="result-note">No alert found only means that no matching record was found in the available dataset. This does not guarantee the medicine is safe, genuine or free from quality issues.</p>
        <div class="result-actions">
          <button class="btn btn-outline" type="button" data-print>🖨 Print result</button>
        </div>
      </article>`;
  ...
  function renderResult(outcome) {
    ...
    if (outcome.type === "alert") {
      ...
      html = renderNoAlertResult(outcome.nameDisplay);
      status = "CLEAR";
    } else {
